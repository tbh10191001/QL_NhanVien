...
class Ui_tableRole(object):
    ...
    def checkUnique(self, test):
        ...
        for i in temp:
            for j in i:
                val = str(j)
                if test.casefold().strip() == val.casefold().strip():
                    print("Sai")
                    return False;
        return True
    ...
    def checkAvailable(self):
        ...
        idRole = self.idRole.text()
        cursor = conn.cursor()
        sql = cursor.execute("SELECT COUNT(IDROLE) FROM ACCOUNT WHERE ACCOUNT.IDROLE = '" + idRole + "'")
        count = sql.fetchone()
        return count[0];
    def clickToAdd(self):
        try:
            ...
            rs = re.match(pattern, inputRole)
            if not rs:
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Lỗi', 'Tên quyền phải là chữ và không được bỏ trống.')
                return;
            elif self.checkUnique(inputRole) == False:
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Lỗi', 'Tên quyền bị trùng. Vui lòng chỉnh sửa.')
                return;
            else:
                cursor = conn.cursor()
    ...
    def clickToDel(self):
        try:
            ...
            if self.checkAvailable() == 0:
                ...
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Lỗi', 'Tên quyền tồn tại nhân viên. Không thể xóa.')
                return;
        except:
            traceback.print_exc()
