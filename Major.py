...
class Ui_MainWindow(object):
    ...
    def clickToEdit(self):
        try:
            ...
            rs = re.match(pattern, inputMajor)
            if not rs:
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Loi', 'Ten chuyen nganh khong duoc bo trong va phai la chu')
                return;
            elif self.checkUnique(inputMajor) == False:
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Loi', 'Ten chuyen nganh bi trung. Vui long chinh sua')
                return;
            else:
                cursor = conn.cursor()
                cursor.execute("UPDATE MAJOR SET MAJOR = '" + inputMajor + "' WHERE MAJOR.IDMAJOR = '" + str(idMajor) + "'")
                cursor.commit()
                self.setTableRow()
                self.loadData()
        except:
    ...
    def checkUnique(self, test):
        ...
        conn = pyodbc.connect("DRIVER={SQL Server Native Client 11.0};"
                              ...
                              "MARS_Connection=yes")
        cursor = conn.cursor()
        sql = cursor.execute("SELECT MAJOR FROM MAJOR")
        temp = sql.fetchall()
        for i in temp:
            for j in i:
                # print(j)
    ...
    def checkAvailable(self):
        ...
        idMajor = self.idMajor.text()
        cursor = conn.cursor()
        sql = cursor.execute("SELECT COUNT(IDMAJOR) FROM EMPLOYEE WHERE EMPLOYEE.IDMAJOR = '" + idMajor + "'")
        count = sql.fetchone()
        return count[0];
    ...
    def clickToAdd(self):
        try:
            ...
            if not rs:
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Lỗi', 'Tên chuyên ngành phải là chữ và không được bỏ trống.')
                return;
            elif self.checkUnique(inputMajor) == False:
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Lỗi', 'Tên chuyên ngành bị trùng. Vui lòng chỉnh sửa.')
                return;
            else:
                cursor = conn.cursor()
    ...
    def clickToDel(self):
        try:
            ...
            if self.checkAvailable() == 0:
                ...
                master = Tk()
                master.withdraw()
                messagebox.showinfo('Lỗi', 'Chuyên ngành tồn tại nhân viên. Không thể xóa.')
                return;
        except:
            traceback.print_exc()
